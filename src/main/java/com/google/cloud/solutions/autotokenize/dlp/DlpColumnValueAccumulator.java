...
public class DlpColumnValueAccumulator
  ...
  private HashMap<String, LinkedList<Value>> values = Maps.newHashMap();
  ...
  public boolean addElement(KV<String, Iterable<Value>> newColumnValues) {
    ...
    Iterables.addAll(newValues, newColumnValues.getValue());
    ...
    if (accumulatedSize + newSize <= MAX_SERIALIZED_SIZE
      ...
      if (!values.containsKey(newColumnValues.getKey())) {
        values.put(newColumnValues.getKey(), newLinkedList());
      }
  ...
  public Batch<KV<Table, Map<String, String>>> makeBatch() {
    var allHeaders = ImmutableList.copyOf(values.keySet());
    ...
    var valueRows =
        ZippingIterator.createElementStream(allValues)
            .map(
                valueList ->
                    valueList.stream()
                        .map(value -> value.orElse(Value.getDefaultInstance()))
                        .collect(Collectors.toList()))
            .map(valueList -> Table.Row.newBuilder().addAllValues(valueList).build())
            .collect(toList());
  ...
  private static class ConvertEmptyElementsToDefault
    ...
    @Override
    public List<Value> apply(@Nullable List<Optional<Value>> elements) {
      if (elements == null) {
        return List.of();
      }
